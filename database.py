...
def load_history() -> List[Dict[str, Any]]:
    ...
    for row in rows:
        history.append({
            ...
            'total_with_deposit': row['total_with_deposit']
        })
    return history
...
def save_history_entry(date: str, portfolio_value: float, total_with_deposit: float):
    ...
    cursor.execute("SELECT id FROM portfolio_history WHERE date = ?", (date,))
    existing = cursor.fetchone()
    ...
    if existing:
        ...
        cursor.execute("""
            ...
            SET portfolio_value = ?, total_with_deposit = ?
            WHERE date = ?
...
# ===== Функции миграции данных из JSON в SQLite =====
